...
class WorkspaceIL:
    ...
    def train_il(self):
        ...
        metrics = None
        tmp_step = 0
        while seed_until_step(self.global_step):
            if time_step.last():
                ...
                if self.cfg.suite.save_snapshot and (self.global_step % 50000 == 0):
            ...
            if not seed_until_step(self.global_step):
                ...
                metrics = self.agent.update(self.replay_iter, self.expert_replay_iter, 
                                            self.global_step, self.cfg.bc_regularize, exp_demo)
        ...
        # with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.CUDA], profile_memory=True, record_shapes=True) as prof:
        with torch.profiler.profile(activities=[torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.CUDA], with_stack=True, profile_memory=True,
            ...
            while train_until_step(self.global_step):
                ...
                if self.global_step-tmp_step > 1103:
                    print("break out")
                    break
...
def main(cfg):
    from train_profiler import WorkspaceIL as W
    root_dir = Path.cwd()
    workspace = W(cfg)
