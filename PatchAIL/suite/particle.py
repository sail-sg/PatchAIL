...
class RGBArrayAsObservationWrapper(dm_env.Environment):
	...
	def reset(self, **kwargs):
		obs = {}
		obs['features'] = self._env.reset(**kwargs).astype(np.float32) / self._height
	...
	def step(self, action):
		...
		obs = {}
		obs['features'] = observation.astype(np.float32) / self._height
		obs['pixels'] = self._env.render("rgb_array", width=self._width, height=self._height)
...
def make(name, frame_stack, action_repeat, seed, height, width, block, reward_type='sparse'):
	if block is None:
		block = [[0.1*height,0.3*height,0.1*width,0.3*width], [0.6*height,0.8*height,0.6*width,0.8*width]]
	
	env = gym.make(name, height=height, width=width, start=[0,0], block=block, reward_type=reward_type)
	env.seed(seed)
